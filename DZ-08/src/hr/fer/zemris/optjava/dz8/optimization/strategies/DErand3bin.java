...
public class DErand3bin implements IStrategy {
	private Random random;
	private double F;
	private double cr;

	public DErand3bin(double F, double cr) {
	...
	public Vector generateTrial(List<Vector> population, int targetIndex) {
		...
		Vector mutant = base.add(x1.sub(x2).mul(F)).add(x3.sub(x4).mul(F)).add(x5.sub(x6).mul(F));
